...
def run_agent(
    *,
    ...
    # TODO (LO-6 loop): drive the model with a stop_reason loop, routing every tool call
    #   through the AuditedToolRunner gateway so it lands in the audit trail.
    #   1. gateway = AuditedToolRunner(bridge, trail, clock or SystemClock(), caller_identity)
    #   2. tools = bridge.anthropic_tools(); seed messages = [{"role":"user","content":request}]
    #   3. loop up to max_iterations (a backstop only):
    #        response = runner.create(system=system_prompt, messages=messages, tools=tools)
    #        - if response.stop_reason != "tool_use": return Outcome(stop_reason, final text
    #          joined from the text blocks, tool_calls, audit_entries=len(trail))  <- the
    #          real termination is stop_reason == "end_turn", NOT the iteration cap.
    #        - else: append the assistant turn (_content_to_wire), run each tool_use block
