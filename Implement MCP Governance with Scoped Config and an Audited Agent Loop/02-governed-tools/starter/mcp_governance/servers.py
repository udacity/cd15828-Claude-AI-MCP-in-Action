...
FastMCP``, ``@server.tool``, ``@server.resource(uri)``) -- they are NOT in the Anthropic
cookbook, whose example servers hand-roll JSON-RPC and are tool-only. Use the SDK.
"""
...
def _not_found(kind: str, identifier: str) -> dict[str, Any]:
    ...
    # TODO (LO-5 / Graceful Tool Failure): return a STRUCTURED error dict, never raise.
...
def build_claims_server(service: ClaimsService | None = None) -> FastMCP:
    ...
    # TODO (LO-3): expose the claims schema as an MCP Resource at SCHEMA_RESOURCE_URI so
    #   agents discover available fields/enums WITHOUT burning exploratory get_claim calls.
